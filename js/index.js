        ...
        if (DNI > 46000000) {
            ...
            if (TIEMPO >= 24) {
                ...
                if (respuesta_plan_de_pago.toLowerCase() === "si") {
                    ...
                    alert(`Usted deberá pagar la suma total de ${monto_final} cumplido el plazo de ${TIEMPO} meses`);
                }
            }
            const datosUsuario = {
                nombre: NOMBRE,
...
function cargarDatosDesdeJSON() {
    fetch('../datosUsuarios.json')
        ...
        .then(data => {
            ...
            simularPrestamoDesdeJSON(datosUsuario);
        })
        .catch(error => {
            console.error('Error al cargar datos desde el archivo JSON:', error);
            alert('Error al cargar datos desde el archivo JSON');
        });
}

function simularPrestamoDesdeJSON(datosUsuario) {
}
